...
function copyBordero(btn){ copyToClipboard(btn, 'bordero'); }
...
const SORT_MAP = ['','','empresa','tipo','num','parcela','razao','fornecedor','codFornec','emissao','vencimento','vencReal','valor','saldo','status','natureza','baixa','historico','valLiqBaix','dtLiberacao','tipoPgto','cnpj','uuid',''];
...
function render(resetPage=false){
  ...
  document.getElementById('tb').innerHTML = pageRows.map(r=>`
    <tr>
      ...
      <td style="text-align:center"><button type="button" class="det-btn" onclick="toggleDet(${r.id},this)" title="Ver detalhes do título">▸</button></td>
...
// === Dropdown de detalhes por título (aba Todos os Títulos) ===
function buildDetRow(r){
  const i=splitUserDate(r.usrInc), alt=splitUserDate(r.usrAlt);
  const multa=Number(r.multa)||0, juros=Number(r.juros)||0;
  return `<tr class="det-row" id="det-${r.id}" style="display:none"><td colspan="24">
    <div class="det-grid">
      <div class="det-grp det-grp-inc"><span class="det-grp-t">👤 Inclusão</span><div class="det-item"><span class="det-l">Nome Inc</span><span class="det-v">${escHtml(i.name)||'—'}</span></div><div class="det-item"><span class="det-l">Data Inc</span><span class="det-v">${i.date||'—'}</span></div></div>
      <div class="det-grp det-grp-alt"><span class="det-grp-t">✏️ Alteração</span><div class="det-item"><span class="det-l">Nome Alt</span><span class="det-v">${escHtml(alt.name)||'—'}</span></div><div class="det-item"><span class="det-l">Data Alt</span><span class="det-v">${alt.date||'—'}</span></div></div>
      <div class="det-grp det-grp-brd"><span class="det-grp-t">📋 Borderô</span><div class="det-item"><span class="det-l">Nº Borderô</span><span class="det-v">${r.numBordero?`<button type="button" class="copy-mini" data-bordero="${escHtml(r.numBordero)}" onclick="copyBordero(this)" title="Copiar Nº Borderô: ${escHtml(r.numBordero)}">${escHtml(r.numBordero)} 📋</button>`:'—'}</span></div><div class="det-item"><span class="det-l">Dt. Borderô</span><span class="det-v">${fmtDt(r.dtBordero)}</span></div></div>
      <div class="det-grp det-grp-bx"><span class="det-grp-t">✅ Baixa</span><div class="det-item"><span class="det-l">Val. Líq. Baixa</span><span class="det-v">${fmtBR(r.valLiqBaix||0)}</span></div><div class="det-item"><span class="det-l">Dt. Baixa</span><span class="det-v">${fmtDt(r.baixa)}</span></div></div>
      <div class="det-grp det-grp-enc"><span class="det-grp-t">💰 Encargos</span><div class="det-item"><span class="det-l">Multa</span><span class="det-v det-multa">${fmtBR(multa)}</span></div><div class="det-item"><span class="det-l">Juros</span><span class="det-v det-juros">${fmtBR(juros)}</span></div><div class="det-item"><span class="det-l">Multa + Juros</span><span class="det-v det-total">${fmtBR(multa+juros)}</span></div></div>
    </div></td></tr>`;
}
